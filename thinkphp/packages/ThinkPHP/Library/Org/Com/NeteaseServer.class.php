...
class NeteaseServer{
    ...
    public function __construct(){
        $this->AppKey    = C('Netease.AppKey');
        $this->AppSecret = C('Netease.AppSecret');
    ...
    public function postDataCurl($url,$data){
		try {
			$CheckSum = $this->checkSumBuilder();       //发送请求前需先生成checkSum
			...
			//$meta = curl_getinfo($ch);
			 \Think\Log::record($result,'YUNXIN',true);
			 //\Think\Log::record($meta['request_header'],'YUNXIN',true);
			curl_close($ch);
			if (false === $result) {
     ...
     */
    public function createUserIds($accid,$name='',$icon='',$props='{}',$token=''){
     ...
     * @return $result      [返回array数组对象]
     */
    public function updateUinfo($accid,$data){
